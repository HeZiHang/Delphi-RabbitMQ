  ...
  begin
    ...
    begin
      if ((AMQP_RESPONSE_LIBRARY_EXCEPTION = ret.reply_type) and
          (AMQP_STATUS_UNEXPECTED_STATE = amqp_status_enum(ret.library_error))) then
      begin
        if (AMQP_STATUS_OK <> amqp_status_enum(amqp_simple_wait_frame(conn, @frame))) then Exit;
          ...
          case (frame.payload.method.id) of
            ...
            AMQP_BASIC_RETURN_METHOD:
              ...
              begin
                ...
                if (AMQP_RESPONSE_NORMAL <> ret.reply_type) then Exit;
...
begin
  ...
  if  status<>0 then die('opening TCP socket', []);
  ...
  die_on_amqp_error(amqp_get_rpc_reply(conn), 'Opening channel');
    ...
    r := amqp_queue_declare(conn, 1, amqp_empty_bytes, 0, 0, 0, 1,
                                 amqp_empty_table);
  ...
  amqp_queue_bind(conn, 1, queuename, amqp_cstring_bytes(PAnsiChar(exchange)), amqp_cstring_bytes(PAnsiChar(bindingkey)),
                  amqp_empty_table);
